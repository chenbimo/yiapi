...
<div class="ui text container" style="max-width: none !important;">
    <div class="ui floating message">
        ...
        <h2 class="ui header">
            ...
            <div class="content">
                <?php echo $service; ?>
                <div class="sub header"><?php echo $description; ?>
                </div>
            </div>
        </h2>

        <h4><i class="linkify in alternate icon"></i><?php echo \PhalApi\T('API Url'); ?>：<?php echo $url; ?>?s=<?php echo $service; ?>
        </h4>

        <div class="ui raised segment">
            <span class="ui red ribbon label"><?php echo \PhalApi\T('API Docs'); ?></span>
            <div class="ui message">
                <p><?php echo \PhalApi\T('API Description'); ?>：<?php echo $descComment; ?>
                </p>
            </div>
        </div>

        <h3><i class="sign in alternate icon"></i><?php echo \PhalApi\T('API Parameters'); ?>
        </h3>
        <table class="ui red celled striped table">
            <thead>
                <tr>
                    <th><?php echo \PhalApi\T('Parameter Name'); ?>
                    </th>
                    <th><?php echo \PhalApi\T('Type'); ?>
                    </th>
                    <th><?php echo \PhalApi\T('Is Required'); ?>
                    </th>
                    <th><?php echo \PhalApi\T('Default'); ?>
                    </th>
                    <th><?php echo \PhalApi\T('Note'); ?>
                    </th>
                    <th><?php echo \PhalApi\T('Description'); ?>
                    </th>
                </tr>
            </thead>
            <tbody>
...
foreach ($rules as $key => $rule) {
    ...
    // 数组类型的格式说明
    if ('array' == $rule['type'] && in_array($rule['format'], ['json', 'explode'])) {
        $type .= sprintf(
...
?>
        ...
        <h3><i class="sign out alternate icon"></i><?php echo \PhalApi\T('Parameter Name'); ?>
        </h3>
        <table class="ui green celled striped table">
            <thead>
                <tr>
                    <th><?php echo \PhalApi\T('Response Result'); ?>
                    </th>
                    <th><?php echo \PhalApi\T('Type'); ?>
                    </th>
                    <th><?php echo \PhalApi\T('Description'); ?>
...
?>
                ...
                <table class="ui red celled striped table">
                    <thead>
                        <tr>
                            <th width="25%"><?php echo \PhalApi\T('Parameter'); ?>
                            </th>
                            <th width="10%"><?php echo \PhalApi\T('Is Required'); ?>
                            </th>
                            <th width="65%"><?php echo \PhalApi\T('Value'); ?>
                            </th>
                        </tr>
                    </thead>
                    <tbody id="params">
                        <tr>
                            <td>service</td>
                            <td>
                                <font color="red"><?php echo \PhalApi\T('Required'); ?>
                                </font>
                            </td>
                            <td>
                                <div class="ui fluid input disabled"><input name="service" data-source="get" value="<?php echo $service; ?>" style="width:100%;" class="C_input" /></div>
                            </td>
                        </tr>
                        <?php
foreach ($rules as $key => $rule) {
    ...
    if ('server' == $source || 'header' == $source) {
        continue;
    }
    $name = $rule['name'];
    $require = isset($rule['require']) && $rule['require'] ? '<font color="red">' . \PhalApi\T('Required') . '</font>' : \PhalApi\T('Optional');
    ...
    $default = isset($rule['default'])
        ? (is_array($rule['default']) // 针对数组，进行反序列化
            ? (!empty($rule['format']) && 'explode' == $rule['format']
    ...
    $inputType = (isset($rule['type']) && 'file' == $rule['type']) ? 'file' : 'text';
...
-->
                ...
                <h3><i class="code icon"></i><?php echo \PhalApi\T('Client Request Demo'); ?>
                </h3>
...
foreach ([
    ...
    if (file_exists($codeFile)) {
        ...
        $demoCodes[$whatCode] = str_replace(['{url}', '{s}'], [$url, $service], $demoCodes[$whatCode]);
    }
}
...
$thisYear = date('Y');
$tips = \PhalApi\T('Tips: ');
$helpMsg = \PhalApi\T('This API Document will be generated automately by PHP code and comments. More detail please visit <a href="{url}" target="_blank">Docs</a>.', ['url' => 'http://docs.phalapi.net/#/v2.0/api-docs']);
echo <<<EOT
