...
<head>
    <meta charset="utf-8">
    <title><?php echo $projectName; ?> - <?php echo \PhalApi\T('Online API Docs'); ?>
    </title>
        ...
        ?>
            ...
            if ('fold' == $theme) {
                ...
                foreach ($allApiS as $namespace => $subAllApiS) {
                    ...
                    echo sprintf('<div class="content %s" style="margin-left:-16px;margin-right:-16px;margin-bottom:-13px;">', 0 == $num ? 'active' : '');
                    // 每个命名空间下的接口类
                    foreach ($subAllApiS as $key => $item) {
                        echo sprintf('<a class="item %s" data-tab="%s">%s</a>', 0 == $num ? 'active' : '', str_replace('\\', '_', $namespace) . $key, $item['title']);
                        ++$num;
                    }
                        ...
                        foreach ($subAllApiS as $key => $item) {
                            ...
                            foreach ($item['methods'] as $mKey => $mItem) {
                                ...
                                if (!empty($_GET['keyword'])) {
                                    ...
                                    if (false !== stripos($mItem['service'], $keyword)) {
                                        $isMatchByKeyword = true;
                                    } elseif (false !== stripos($mItem['title'], $keyword)) {
                                        $isMatchByKeyword = true;
                                    } elseif (false !== stripos($mItem['desc'], $keyword)) {
                                        $isMatchByKeyword = true;
                                    }
                                    // 未匹配，则跳过
                                    if (!$isMatchByKeyword) {
            ...
            ?>
                ...
                foreach ($allApiS as $namespace => $subAllApiS) {
                    foreach ($subAllApiS as $key => $item) {
                        ?>
                        <div class="ui  tab <?php if (0 == $num2) { ?>active<?php } ?>" data-tab="<?php echo str_replace('\\', '_', $namespace) . $key; ?>">
                            <table class="ui red celled striped table celled striped table">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th><?php echo \PhalApi\T('API Service'); ?>
                                        </th>
                                        <th><?php echo \PhalApi\T('API Title'); ?>
                                        </th>
                                        <th><?php echo \PhalApi\T('API Description'); ?>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                        ...
                        foreach ($item['methods'] as $mKey => $mItem) {
                            ...
                            $NO = $num++;
                            echo "<tr><td>{$NO}</td><td><a href=\"./docs/{$link}\" target='_blank'>{$s}</a></td><td>{$mItem['title']}</td><td>{$mItem['desc']}</td></tr>";
                        } ?>
                                </tbody>
                            </table>
